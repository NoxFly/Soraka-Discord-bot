...
module.exports = class Region extends Command {
    ...
    action(client, message, args) {
		// if(args.length == 0) return message.channel.send(new Discord.MessageEmbed().setTitle('Langs').setDescription(Object.keys(client.riotAPI.LANGS).join(', ')));
		// if(Object.keys(client.riotAPI.LANGS).indexOf(args[0].toLowerCase())!==-1) {
