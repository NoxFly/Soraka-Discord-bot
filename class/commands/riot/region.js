...
module.exports = class Region extends Command {
    ...
    action(message, args) {
		if(args.length == 0) return message.channel.send(new Discord.MessageEmbed().setTitle('Regions').setDescription(Object.keys(riotAPI.REGIONS).join(', ')));
		if(Object.keys(riotAPI.REGIONS).indexOf(args[0].toUpperCase())!==-1) {
