...
let basic = [{
        ...
        result: (msg, args) => {
            if (args.length > 0) {
                ...
                for (let i = 0; i < main.commands.length - 1; i++) {
                    let cmdName = main.commands[i].name;
                    if (args[0] == cmdName && main.commands[i].group != "hidden" || args[0] == cmdName && admin(msg.author.id)) {
                        text = "  • `" + cmdName + "` : " +
                            main.commands[i].description + '\n\tWrite → `' + main.commands[i].usage + "`";
                        send(msg, text);
                        return;
                    }
                }
    ...
    */
    {
        ...
        result: (msg) => {
            ...
            send(msg,
                '```*italics*' + (' '.repeat(15)) + '_italics_' + (' '.repeat(15)) + '**bold**\n__underline__' + (' '.repeat(11)) + '~~Strikethrough~~```' +
...

    {
        ...
        result: (msg, args, profile) => {
            ...
            let mpAuth = profile.delay.mpAuth,
                ans = 0;
